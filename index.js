...
for (const now of uptimelink) {
  const monitor = new Monitor({
    website: `${now}`,
    title: '𝑴𝒓𝑼𝒛𝒂𝒊𝒓𝑿𝒙𝑿-𝑴𝑻𝑿',
    interval: 59,
  config: {
    intervalUnits: 'seconds'
  }
});
monitor.on('up', (res) => console.log(chalk.bold.hex("#00FF00")("[ 𝑴𝒓𝑼𝒛𝒂𝒊𝒓𝑿𝒙𝑿-𝑴𝑻𝑿 ] ❯ ") + chalk.hex("#00FF00")(`${res.website}`)))
...
axios.get("https://raw.githubusercontent.com/Mruzair-mtx/uzair/main/package.json").then((res) => {
  logger(res['data']['name'], "[ NAME ]");
  logger("Version: " + res['data']['version'], "[ VERSION ]");
  logger(res['data']['description'], "[ DESCRIPTION ]");
});
startBot();
// THIZ BOT WAS MADE BY  UZAIR RAJPUT - DO NOT STEAL MY CODE (つ ͡ ° ͜ʖ ͡° )つ ✄ ╰⋃╯
app.get('/', (req, res) => res.sendFile(__dirname+'/𝑴𝒓𝑼𝒛𝒂𝒊𝒓𝑿𝒙𝑿.html'))